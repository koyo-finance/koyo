...
event CommitVotingEscrow:
    voting_escrow: indexed(address)
event ApplyVotingEscrow:
    voting_escrow: indexed(address)
...
WEIGHT_VOTE_DELAY: constant(uint256) = 7 * 86400


token: public(address)  # KYO token

# Gauge parameters
# All numbers are "fixed point" on the basis of 1e18
n_gauge_types: public(int128)
n_gauges: public(int128)
gauge_type_names: public(HashMap[int128, String[128]])
...
future_voting_escrow: public(address)
...
def __init__(_token: address, _voting_escrow: address):
    ...
    @param _token `Koyo` contract address
...
@external
def add_type(_name: String[128], weight: uint256 = 0):
    """
...
@external
def commit_voting_escrow(addr: address):
    assert msg.sender == self.owner  # dev: owner only

    self.future_voting_escrow = addr

    log CommitVotingEscrow(addr)


@external
def apply_voting_escrow():
    assert msg.sender == self.owner  # dev: owner only

    _voting_escrow: address = self.future_voting_escrow
    assert _voting_escrow != ZERO_ADDRESS  # dev: ve not set
    self.voting_escrow = _voting_escrow

    log ApplyVotingEscrow(_voting_escrow)
