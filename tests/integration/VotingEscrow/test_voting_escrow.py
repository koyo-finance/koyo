...
def test_voting_powers(web3, chain, accounts, koyo, voting_escrow):
    a0, a1 = accounts[:2]
    amount = 1000 * 10 ** 18
    ...
    koyo.mint_available(a0, {"from": a0})
    assert approx(koyo.totalSupply(), koyo.emission_rate() * DAY, 1)

    koyo.transfer(a1, amount, {"from": a0})
    stages = {}

    koyo.approve(voting_escrow.address, amount * 10, {"from": a0})
    koyo.approve(voting_escrow.address, amount * 10, {"from": a1})
