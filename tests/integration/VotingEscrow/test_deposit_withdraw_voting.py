...
class StateMachine:
    ...
    def rule_create_lock(self, st_account, st_value, st_lock_duration):
        ...
        if st_value == 0:
            ...
            with brownie.reverts("W"):
                self.voting_escrow.create_lock(
                    st_value, unlock_time, {"from": st_account, "gas": GAS_LIMIT}
                )

        elif unlock_time <= chain.time():
            with brownie.reverts("LBF"):
                self.voting_escrow.create_lock(
                    st_value, unlock_time, {"from": st_account, "gas": GAS_LIMIT}
                )

        elif unlock_time > chain.time() + MAXTIME:
            with brownie.reverts("VLABT"):
                self.voting_escrow.create_lock(
                    st_value, unlock_time, {"from": st_account, "gas": GAS_LIMIT}
                )
    ...
    def rule_force_withdraw(self, st_account):
        """
        Forcefully withdraw tokens from the voting escrow.
        """
        if self.voting_balances[st_account]["unlock_time"] < chain.time():
            # fail path - after unlock time
            with brownie.reverts("LE"):
                self.voting_escrow.force_withdraw({"from": st_account, "gas": GAS_LIMIT})
        else:
            # success path - specific amount
            self.voting_escrow.force_withdraw({"from": st_account, "gas": GAS_LIMIT})
            self.voting_balances[st_account]["value"] = self.voting_balances[st_account]["value"] / 5
...
def test_state_machine(state_machine, accounts, VotingEscrow):
    token = ERC20("Koyo", "KOYO", 18)
    voting_escrow = VotingEscrow.deploy(
        token, "Voting-escrowed KOYO", "veKOYO", "veKOYO_1", {"from": accounts[0]}
    )

    state_machine(StateMachine, accounts[:10], token, voting_escrow, settings={"max_examples": 3})
